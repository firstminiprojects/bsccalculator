document.addEventListener('DOMContentLoaded', () => {
  ...
  // 2. Goal Configurations & System Settings
  const metricsGoal = {
  ...
  // Adjust overall font sizing inside graphics
  if (resultDisplay) {
  ...
  // Inject additional functional CSS animations dynamically
  ...
  // 4. Centered Note Popup Logic + Attention Stopping Handler
  if (noteBtn && noteTooltip) {
      noteBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          
          // Permanently remove animation once clicked
          noteBtn.classList.remove('attention-shaking');
          noteTooltip.classList.toggle('is-visible');
      });
      ...
      document.addEventListener('click', (e) => {
          if (!noteTooltip.contains(e.target) && e.target !== noteBtn) {
              noteTooltip.classList.remove('is-visible');
          }
      });
  ...
  function getProgressiveColor(score) {
    ...
    const red = [239, 68, 68], orange = [249, 115, 22], blue = [59, 130, 246];     
  ...
  [fcrInput, crsInput, ahtInput, qaInput].forEach(input => {
    if (input) input.addEventListener('input', () => resetInputError(input));
  });

  // 6. UI Missing Data Alerter
  function showInputError(missingMetrics) {
    ...
    Object.assign(alertDiv.style, {
        ...
        background: '#fef2f2', border: '1px solid #fee2e2', borderLeft: '4px solid #ef4444',
        borderRadius: '8px', padding: '12px 16px', marginBottom: '20px', width: '100%',
        boxSizing: 'border-box', animation: 'alertSlideIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)', opacity: '1'
    });
    ...
    alertDiv.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
            ...
            <span style="color: #991b1b; font-size: 0.8rem; font-weight: 600; line-height: 1.4;">
                Missing values for: <strong>${missingMetrics.join(', ')}</strong>. Please enter them to compute score.
            </span>
        </div>
    ...
    function dismissAlert() {
        if (alertDiv && alertDiv.parentNode) {
            ...
            alertDiv.addEventListener('animationend', () => { if (alertDiv.parentNode) alertDiv.remove(); });
        }
    }
  }

  // 7. Performance Assessment Framework
  function generatePerformanceTips(fcr, crs, aht, qa, isSuccess) {
    let tipsHtml = '';
    const gridWrapper = `display: grid; grid-template-columns: repeat(auto-fit, minmax(min(100%, 320px), 1fr)); gap: 16px; margin-top: 16px; width: 100%; box-sizing: border-box; align-items: stretch;`;
    const getCardStyle = (accentColor) => `background: #ffffff; border: 1px solid #e2e8f0; border-left: 3.5px solid ${accentColor}; border-radius: 8px; box-sizing: border-box; width: 100%; display: flex; flex-direction: column; justify-content: space-between; min-height: 84px; cursor: pointer; transition: transform 0.25s cubic-bezier(0.16, 1, 0.3, 1), box-shadow 0.25s cubic-bezier(0.16, 1, 0.3, 1), background-color 0.25s ease, border-color 0.25s ease;`;
    const headerRowStyle = `display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; width: 100%; box-sizing: border-box; gap: 16px; height: 100%;`;
    const iconBox = (bg, color) => `height: 36px; width: 36px; border-radius: 6px; background: ${bg}; color: ${color}; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: transform 0.2s ease;`;
                ...
                modules += `
                    <div onclick="document.getElementById('modal-${index}').showModal()" 
                         ...
                         onmouseenter="
                            ...
                            this.style.borderColor = this.getAttribute('data-metric-color');
                    ...
                    >
                        ...
                        ">
                            ...
                            <div style="display: flex; justify-content: flex-end;">
                                <button onclick="document.getElementById('modal-${index}').close()" style="padding: 6px 14px; font-size: 0.75rem; border-radius: 4px; font-weight: 600; cursor: pointer; border: 1px solid #e2e8f0; background: #ffffff; color: #475569;">Dismiss</button>
                            </div>
                        </dialog>
                    </div>
                `;
  ...
  function triggerConfetti() {
    ...
    for (let i = 0; i < 35; i++) {
        ...
        piece.style.background = ['#4f46e5', '#10b981', '#f59e0b', '#ec4899', '#3b82f6'][Math.floor(Math.random() * 5)];
  ...
  function performCalculation() {
    ...
    progressFillCircle.style.strokeDashoffset = circumference - (boundedVisualScore / 100 * circumference);
    ...
    if (isSuccess) {
        Object.assign(statusBadge.style, {
            backgroundColor: "#f0fdf4", color: "#15803d", border: "1px solid #dcfce7",
        ...
        triggerConfetti();
        achievementMessage.innerHTML = `<span style="color: #475569; font-size: 0.8rem;"><strong>Mastery Status:</strong> Overachievement unlocked. Targets fully exceeded.</span>`;
    } else {
        statusBadge.innerHTML = `
            <button id="scrollToTipsBtn" style="display: inline-flex; align-items: center; gap: 5px; padding: 4px 10px; font-size: 0.675rem; border-radius: 30px; font-weight: 700; cursor: pointer; border: 1px solid #ef4444; background: #ef4444; color: #ffffff; box-shadow: 0 4px 12px rgba(239, 68, 68, 0.25); transition: opacity 0.2s, transform 0.2s; opacity: 0; transform: scale(0.6); pointer-events: none; outline: none;">
                <i data-lucide="info" style="width: 11px; height: 11px; stroke-width: 2.5px;"></i>
                Factors
            </button>
        `;
        Object.assign(statusBadge.style, { backgroundColor: "transparent", border: "none", padding: "0", display: "inline-block" });

        achievementMessage.innerHTML = `
            <div style="position: relative; padding-top: 36px; width: 100%; box-sizing: border-box;">
                <span id="swappedStatusBadge" style="position: absolute; top: -16px; right: 0; display: inline-block; background-color: transparent; color: #64748b; border: none; padding: 0; border-radius: 0; font-family: 'Playfair Display', Georgia, serif; font-style: italic; font-weight: 500; font-size: 0.725rem; box-shadow: none; letter-spacing: 0.01em;">
                    Progress over perfection. Never give up.
                </span>
                <span style="color: #475569; font-size: 0.8rem; line-height: 1.45; display: block;">
                    <strong>Action Plan:</strong> Focus on the metrics below to improve your scorecard.
                </span>
            </div>
        `;
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
        inputs.forEach(i => { if(i) { i.value = ''; resetInputError(i); } });
        resultContainer.classList.remove('show');
        performanceTips.style.display = 'none';
        metricsLegendList.innerHTML = '';
        const oldAlert = document.getElementById('inputValidationError');
        if (oldAlert) oldAlert.remove();
        progressFillCircle.style.strokeDashoffset = 2 * Math.PI * 92;
